...
export default function EditBlog() {
  ...
  const fetchBlog = async () => {
    try {
      ...
      if (blog.mdFiles && blog.mdFiles.length > 0) {
        const mdResponse = await fetch(`http://localhost:5000/api/blogs/${blogId}/md-content`);
  ...
  const uploadAssets = async (files: FileList) => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:5000/api/blogs/${blogId}/assets`, {
        method: 'POST',
        body: formData
      });
  ...
  const uploadCover = async (file: File) => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:5000/api/blogs/${blogId}/cover`, {
        method: 'POST',
        body: formData
      });
  ...
  const deleteAsset = async (index: number) => {
    ...
    try {
      const response = await fetch(`http://localhost:5000/api/blogs/${blogId}/assets/${index}`, {
        method: 'DELETE'
      });
  ...
  const updateAssetName = async (index: number, newName: string) => {
    try {
      const response = await fetch(`http://localhost:5000/api/blogs/${blogId}/assets/${index}/name`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
